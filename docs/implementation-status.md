...
This snapshot describes checked-in GitHub code, not a separate local package or design bundle.
...
| Peak comparison and rollback | Planned | config/model fields and architecture docs | Candidate is currently always marked promoted |
| Plateau/early stopping | Planned | config fields only | No state transition uses them |
| Regression audit CLI | Planned | README target only | `audit` command is absent |
| Model/Harness co-evolution | Planned | config fields and docs | `harness/` contains only `__init__.py`; `coevolve` command is absent |
| Human approval boundary | Planned | production checklist | No approval state or CLI |
...
The following target states are not reachable yet:

```text
DIAGNOSED, HYPOTHESIS_SELECTED, DATASET_REVIEW_PENDING,
PEAK_COMPARED, PROMOTED, REJECTED, ROLLED_BACK, PLATEAU_STOPPED,
REGRESSION_AUDITED, HARNESS_MUTATED, HARNESS_ACCEPTED,
TRACE_HARVESTED, MODEL_REVIEW_PENDING, HOT_SWAPPED
```
