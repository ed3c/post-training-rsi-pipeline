...
def verify_bundle(bundle: str | Path) -> BundleVerificationReport:
    ...
    # A bundle is only reproducible if its root holds exactly the manifest and
    # the blob store. An extra entry is unaccounted content, so it fails closed
    # rather than being ignored during restore.
...
def stage_bundle(
    ...
    try:
        ...
        staging_root.mkdir(mode=0o700)
        # Recreate the tree with private writable modes first: a source
        # directory recorded as read-only cannot have its own files written
        # into it. Shallow-to-deep on the way in, then the manifest's real
        # modes deep-to-shallow once every file exists.
...
def _verify_directory_against_manifest(
    ...
    for entry in manifest.entries:
        ...
        metadata = candidate.lstat()
        # Content alone is not a faithful restore: a widened permission mode is
        # a real difference from the recorded source.
